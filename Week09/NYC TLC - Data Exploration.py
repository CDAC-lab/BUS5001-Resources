...
df_yellow_18_19 = spark.read.load("/bus5001/bigdata/delta/nyc-yellow")
...
# MAGIC %sql
# MAGIC DESCRIBE DETAIL delta. `dbfs:/bus5001/bigdata/delta/nyc-yellow`
...
# MAGIC %sql
# MAGIC SELECT count(*) FROM delta. `dbfs:/bus5001/bigdata/delta/nyc-yellow`;
...
# MAGIC FROM delta. `dbfs:/bus5001/bigdata/delta/nyc-yellow`
# MAGIC GROUP BY
...
display(spark.sql(
    ...
    SELECT
      ...
      ,CASE
        ...
        ELSE 'Voided trip'
      END AS Payment_type
    FROM delta. `dbfs:/bus5001/bigdata/delta/nyc-yellow`
    GROUP BY
...
# MAGIC     FROM delta. `dbfs:/bus5001/bigdata/delta/nyc-yellow` WHERE pickup_datetime BETWEEN "2018-01-01" AND "2018-02-01"
# MAGIC     GROUP BY 
# MAGIC         to_date(pickup_datetime)
# MAGIC         ,pickup_location_id
# MAGIC ) t 
...
# MAGIC FROM
# MAGIC   delta. `dbfs:/bus5001/bigdata/delta/nyc-yellow`
# MAGIC GROUP BY
...
display(spark.sql(
    ...
    SELECT
      ...
      ,to_date(pickup_datetime) date
    FROM delta. `dbfs:/bus5001/bigdata/delta/nyc-yellow` WHERE total_amount >= 0
    GROUP BY
       to_date(pickup_datetime)
    ORDER BY
      to_date(date)
...
display(spark.sql(
    ...
    SELECT
      ...
      ,to_date(pickup_datetime) date
    FROM delta. `dbfs:/bus5001/bigdata/delta/nyc-yellow` WHERE tip_amount >= 0
    GROUP BY
       to_date(pickup_datetime)
    ORDER BY
      to_date(date)
...
display(spark.sql(
    ...
    SELECT
      ...
      ,to_date(pickup_datetime) date
    FROM delta. `dbfs:/bus5001/bigdata/delta/nyc-yellow` WHERE tip_amount >= 0 AND total_amount >= 0
    GROUP BY
       to_date(pickup_datetime)
    ORDER BY
      to_date(date)
